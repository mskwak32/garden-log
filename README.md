...
```
garden-log/
├── app/                  # 앱 진입점 (MainActivity, AppApplication, 알람 수신)
├── feature/
│   ├── plant/            # 식물/일기 화면 UI Layer (Screen, ViewModel, State)
│   └── setting/          # 설정 화면 UI Layer
...
└── core/
    ...
    └── common_ui/        # BaseViewModel, ViewState/Event/Effect, Compose Theme, 공통 UI
```
...
| | PlantDetailScreen → DiaryEditScreen (일기 작성) |
| | PlantDetailScreen → DiaryMoreScreen (일기 더보기) |
| | PlantDetailScreen → DiaryExportScreen → ExportedDiaryListScreen |
| 일기 | DiaryListScreen → DiaryDetailScreen → DiaryEditScreen |
| 설정 | SettingScreen → ExportedDiaryListScreen |
